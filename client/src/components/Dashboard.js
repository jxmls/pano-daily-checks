...
function useKnownIssuesStats() {
  return useMemo(() => {
    try {
      ...
      return { active: active.length, due: due.length, retired: retired.length, showBadge: due.length > 0 };
    } catch {
...
// ---------- Tile metadata ----------
const modules = [
  { id: "solarwinds", title: "SolarWinds Checks", icon: <img src={solarwindsLogo} alt="SolarWinds Logo" className="h-6" /> },
  { id: "vsan",       title: "Vmware vSAN Checks", icon: <img src={vmwareLogo} alt="VMware Logo" className="h-6 object-contain" /> },
  { id: "veeam",      title: "Veeam Backup Checks", icon: <img src={veeamLogo} alt="Veeam Logo" className="h-6 object-contain" /> },
  { id: "checkpoint", title: "Checkpoint Checks",   icon: <img src={checkpointLogo} alt="Checkpoint Logo" className="h-6 object-contain" /> },
  {
    id: "KnownIssues",
    title: "Known Issues",
    icon: (
  ...
  {
    id: "admin",            // ⬅️ was "Snapshots"
    title: "Admin Portal",  // ⬅️ was "Active Snapshots"
    icon: (
...
export default function Dashboard({ onSelectModule }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start p-4 bg-gradient-to-b from-gray-100 via-white to-white text-gray-800">
      ...
      <p className="text-sm text-gray-500 mb-6">Select a module below to begin your daily checks.</p>
          ...
          return (
            ...
            >
              {isKnownIssues && ki.showBadge && (
                <span className="absolute top-2 right-2 px-2 py-0.5 text-[10px] rounded bg-red-100 text-red-700">Review due</span>
              )}
